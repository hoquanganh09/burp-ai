...
class ResponseAnalyzer {
    ...
    private val errorPatterns: Map<VulnClass, List<ErrorPattern>> = mapOf(
        VulnClass.SQLI to listOf(
            ...
            ErrorPattern(Regex("check the manual that corresponds to your (MySQL|MariaDB) server version", RegexOption.IGNORE_CASE), 95, "MySQL version error"),
            ...
            // SQL Server
            ErrorPattern(Regex("Unclosed quotation mark after the character string", RegexOption.IGNORE_CASE), 95, "MSSQL unclosed quote"),
        ...
        VulnClass.STACK_TRACE_EXPOSURE to listOf(
            // Java
            ErrorPattern(Regex("at\\s+[a-zA-Z0-9_.]+\\([a-zA-Z0-9_.]+\\.java:\\d+\\)", RegexOption.IGNORE_CASE), 90, "Java stack trace"),
            ErrorPattern(Regex("java\\.lang\\.(NullPointerException|Exception|RuntimeException)", RegexOption.IGNORE_CASE), 90, "Java exception"),
            // Python
            ...
            // .NET
            ErrorPattern(Regex("at\\s+[a-zA-Z0-9_.]+\\s+in\\s+.*\\.cs:line\\s+\\d+", RegexOption.IGNORE_CASE), 90, ".NET stack trace"),
            ErrorPattern(Regex("System\\.(Web|Data|IO)\\.[A-Z][a-zA-Z]+Exception", RegexOption.IGNORE_CASE), 90, ".NET exception"),
            // PHP
            ErrorPattern(Regex("Fatal error:.*in\\s+/.*\\.php\\s+on\\s+line\\s+\\d+", RegexOption.IGNORE_CASE), 95, "PHP fatal error"),
            ErrorPattern(Regex("Stack trace:.*#\\d+\\s+/.*\\.php\\(\\d+\\)", RegexOption.IGNORE_CASE), 90, "PHP stack trace"),
            // Node.js
            ErrorPattern(Regex("at\\s+[a-zA-Z0-9_$.]+\\s+\\(.*\\.js:\\d+:\\d+\\)", RegexOption.IGNORE_CASE), 90, "Node.js stack trace"),
        ),
        ...
        VulnClass.VERSION_DISCLOSURE to listOf(
            ...
            ErrorPattern(Regex("X-Powered-By:\\s*(PHP|ASP\\.NET|Express)/[0-9.]+", RegexOption.IGNORE_CASE), 85, "Framework version"),
        ...
        VulnClass.GIT_EXPOSURE to listOf(
            ...
            ErrorPattern(Regex("\\[core\\]\\s*\\n.*repositoryformatversion", RegexOption.IGNORE_CASE), HIGH_CONFIDENCE, "Git config"),
        ...
        VulnClass.BACKUP_DISCLOSURE to listOf(
            ...
            ErrorPattern(Regex("^\\s*<\\?php|^\\s*<\\?=", setOf(RegexOption.IGNORE_CASE, RegexOption.MULTILINE)), 90, "PHP source code"),
            ErrorPattern(Regex("^\\s*(from\\s+[a-zA-Z]+\\s+import|import\\s+[a-zA-Z])", setOf(RegexOption.IGNORE_CASE, RegexOption.MULTILINE)), 85, "Python source"),
            ErrorPattern(Regex("^\\s*(public\\s+class|private\\s+void|package\\s+[a-z])", setOf(RegexOption.IGNORE_CASE, RegexOption.MULTILINE)), 85, "Java source"),
        ),
        ...
        VulnClass.DEBUG_EXPOSURE to listOf(
            ...
            ErrorPattern(Regex("phpinfo\\(\\)|PHP Version|Configuration.*PHP", RegexOption.IGNORE_CASE), HIGH_CONFIDENCE, "PHP Info"),
        ...
        VulnClass.SUBDOMAIN_TAKEOVER to listOf(
            ErrorPattern(Regex("There isn't a GitHub Pages site here", RegexOption.IGNORE_CASE), HIGH_CONFIDENCE, "GitHub Pages unclaimed"),
            ErrorPattern(Regex("NoSuchBucket|The specified bucket does not exist", RegexOption.IGNORE_CASE), HIGH_CONFIDENCE, "S3 bucket unclaimed"),
            ...
            ErrorPattern(Regex("The request could not be satisfied.*CloudFront", RegexOption.IGNORE_CASE), 90, "CloudFront unclaimed"),
            ErrorPattern(Regex("Sorry, this shop is currently unavailable.*Shopify", RegexOption.IGNORE_CASE), 90, "Shopify unclaimed"),
        ),

        // ===== S3/Cloud Misconfiguration =====
        VulnClass.S3_MISCONFIGURATION to listOf(
            ErrorPattern(Regex("ListBucketResult|<Contents>.*<Key>", RegexOption.IGNORE_CASE), HIGH_CONFIDENCE, "S3 bucket listing"),
        ...
        VulnClass.CACHE_POISONING to listOf(
            ErrorPattern(Regex("evil-burp-ai-cache\\.com", RegexOption.IGNORE_CASE), HIGH_CONFIDENCE, "Poisoned host in cached response"),
        ),

        // ===== Source Map Disclosure =====
        VulnClass.SOURCEMAP_DISCLOSURE to listOf(
            ErrorPattern(Regex("\"sources\":\\s*\\[.*\\.js\"|\"mappings\":", RegexOption.IGNORE_CASE), HIGH_CONFIDENCE, "Source map content"),
        ...
        VulnClass.OAUTH_MISCONFIGURATION to listOf(
            ErrorPattern(Regex("redirect_uri.*evil-burp-ai\\.com", RegexOption.IGNORE_CASE), HIGH_CONFIDENCE, "OAuth redirect bypass"),
    ...
    private val successPatterns: Map<VulnClass, List<SuccessPattern>> = mapOf(
        VulnClass.XSS_REFLECTED to listOf(
            ...
            SuccessPattern(
                ...
                HIGH_CONFIDENCE, "Script tag with unique marker"
            ),
            SuccessPattern(
                Regex("<[a-z]+[^>]+on(error|load|click|mouseover)\\s*=\\s*['\"]?alert\\s*\\(['\"]?XSS-BURP-AI-1337", RegexOption.IGNORE_CASE),
                HIGH_CONFIDENCE, "Event handler with unique marker"
            ),
            SuccessPattern(
                Regex("<svg[^>]+onload\\s*=\\s*['\"]?alert\\(['\"]?XSS-BURP-AI-1337", RegexOption.IGNORE_CASE),
                HIGH_CONFIDENCE, "SVG onload with marker"
            ),
            SuccessPattern(
                Regex("<img[^>]+onerror\\s*=\\s*['\"]?alert\\(['\"]?XSS-BURP-AI-1337", RegexOption.IGNORE_CASE),
                HIGH_CONFIDENCE, "IMG onerror with marker"
            ),
            ...
            SuccessPattern(
                ...
                75, "Script tag with alert(1) - needs manual verification"
            ),
        ),
        ...
        VulnClass.XSS_DOM to listOf(
            SuccessPattern(
                ...
                90, "DOM XSS marker reflected"
            ),
        ),
        
        VulnClass.OPEN_REDIRECT to listOf(
            // Must be in Location header, not just body content
            SuccessPattern(Regex("^Location:\\s*(https?:)?//evil\\.com", setOf(RegexOption.MULTILINE, RegexOption.IGNORE_CASE)), 95, "Redirect to evil.com"),
        ),
        
        VulnClass.HEADER_INJECTION to listOf(
            SuccessPattern(Regex("^X-Injected:\\s*burp-ai-test", setOf(RegexOption.MULTILINE, RegexOption.IGNORE_CASE)), HIGH_CONFIDENCE, "Injected header"),
        ),
        
        VulnClass.CRLF_INJECTION to listOf(
            SuccessPattern(Regex("^Set-Cookie:\\s*burp-ai-test=true", setOf(RegexOption.MULTILINE, RegexOption.IGNORE_CASE)), HIGH_CONFIDENCE, "CRLF cookie injection"),
        ),
        ...
        VulnClass.CORS_MISCONFIGURATION to listOf(
            SuccessPattern(Regex("^Access-Control-Allow-Origin:\\s*\\*", setOf(RegexOption.MULTILINE, RegexOption.IGNORE_CASE)), 85, "ACAO wildcard"),
            SuccessPattern(Regex("^Access-Control-Allow-Origin:\\s*https?://evil\\.com", setOf(RegexOption.MULTILINE, RegexOption.IGNORE_CASE)), HIGH_CONFIDENCE, "ACAO reflects evil.com"),
            SuccessPattern(Regex("^Access-Control-Allow-Origin:\\s*null", setOf(RegexOption.MULTILINE, RegexOption.IGNORE_CASE)), 90, "ACAO null"),
            SuccessPattern(Regex("^Access-Control-Allow-Credentials:\\s*true", setOf(RegexOption.MULTILINE, RegexOption.IGNORE_CASE)), 85, "Credentials allowed"),
        ),
        ...
        VulnClass.HOST_HEADER_INJECTION to listOf(
            ...
            SuccessPattern(Regex("localhost[^a-zA-Z].*reset|password.*localhost", RegexOption.IGNORE_CASE), 90, "Localhost in password reset"),
        ),
        ...
        VulnClass.CACHE_POISONING to listOf(
            ...
            SuccessPattern(Regex("^X-Cache:\\s*(HIT|hit)", setOf(RegexOption.MULTILINE, RegexOption.IGNORE_CASE)), 85, "Response cached"),
        ),

        // ===== Cache Deception =====
        VulnClass.CACHE_DECEPTION to listOf(
            // Look for sensitive data in responses with static extensions
            SuccessPattern(Regex("email.*@|password|credit.*card|ssn|api.*key", RegexOption.IGNORE_CASE), 85, "Sensitive data potentially cached"),
        ),

        // ===== Git Exposure =====
        VulnClass.GIT_EXPOSURE to listOf(
            SuccessPattern(Regex("ref:\\s*refs/heads/(main|master|develop)", RegexOption.IGNORE_CASE), HIGH_CONFIDENCE, "Git HEAD file"),
            SuccessPattern(Regex("\\[core\\]\\s*\\n\\s*repositoryformatversion", RegexOption.IGNORE_CASE), HIGH_CONFIDENCE, "Git config"),
            SuccessPattern(Regex("\\[remote\\s+\"origin\"\\]\\s*\\n\\s*url\\s*=", RegexOption.IGNORE_CASE), HIGH_CONFIDENCE, "Git remote config"),
        ),
        ...
        VulnClass.BACKUP_DISCLOSURE to listOf(
            ...
            SuccessPattern(Regex("^package\\s+[a-z]+\\.[a-z]+", setOf(RegexOption.MULTILINE, RegexOption.IGNORE_CASE)), 85, "Java package declaration"),
        ),
        ...
        // ===== Source Map Disclosure =====
        VulnClass.SOURCEMAP_DISCLOSURE to listOf(
            SuccessPattern(Regex("\"version\"\\s*:\\s*3,\\s*\"sources\"", RegexOption.IGNORE_CASE), HIGH_CONFIDENCE, "Source map v3"),
            SuccessPattern(Regex("\"sourcesContent\"\\s*:\\s*\\[", RegexOption.IGNORE_CASE), HIGH_CONFIDENCE, "Source map with content"),
        ),

        // ===== Subdomain Takeover =====
        VulnClass.SUBDOMAIN_TAKEOVER to listOf(
            SuccessPattern(Regex("There isn't a GitHub Pages site here", RegexOption.IGNORE_CASE), HIGH_CONFIDENCE, "GitHub Pages takeover"),
    ...
    private val falsePositiveIndicators: Map<VulnClass, List<Regex>> = mapOf(
        VulnClass.SQLI to listOf(
    ...
    data class ErrorPattern(val regex: Regex, val confidence: Int, val evidence: String)
    data class SuccessPattern(val regex: Regex, val confidence: Int, val evidence: String)
    ...
    fun analyze(
        ...
        val modifiedHeaders = modified.response()?.headers()?.map { "${it.name()}: ${it.value()}" }?.joinToString("\n") ?: ""
        val fullResponse = modifiedHeaders + "\n\n" + modifiedBody
        val originalHeaders = original.response()?.headers()?.map { "${it.name()}: ${it.value()}" }?.joinToString("\n") ?: ""
        ...
        val confirmation = when (payload.detectionMethod) {
    ...
    private fun isFalsePositive(vulnClass: VulnClass, response: String): Boolean {
        ...
        for (pattern in patterns) {
            ...
            if (match != null) {
                ...
                return createConfirmation(
                    original, modified, payload, vulnClass,
                    pattern.confidence,
     ...
     */
    private fun isInSafeContext(matchValue: String, fullResponse: String): Boolean {
    ...
    private fun analyzeBooleanBased(
        ...
        val isTrueCondition = payload.value.contains("1'='1") ||
                             payload.value.contains("1=1") ||
                             payload.expectedEvidence.contains("Same response", ignoreCase = true)

        val isFalseCondition = payload.value.contains("1'='2") ||
        ...
        if (isFalseCondition) {
            ...
            val hasSignificantDiff = originalStatus != modifiedStatus ||
    ...
    fun analyzeBooleanBasedDual(
        ...
        if (trueIsSimilarToOriginal && falseIsDifferentFromOriginal && trueAndFalseAreDifferent) {
            ...
            if (totalConfidence >= MIN_CONFIDENCE_TO_REPORT) {
                return createConfirmation(
                    original, falseResponse, falsePayload, vulnClass,
                    totalConfidence,
     ...
     */
    fun calculateDifference(original: String, modified: String): ResponseDiff {
