...
data class AgentSettings(
    ...
    val openAiCompatibleUseOpenRouter: Boolean,
    val openAiCompatibleModels: List<String>,
    ...
    val promptTemplateProfile: String,
    val promptTemplateCustom: String,
    ...
    val passiveAiEnabled: Boolean = true,
...
class AgentSettingsRepository(api: MontoyaApi) {
    ...
    fun load(): AgentSettings {
        ...
        val useOpenRouter = prefs.getBoolean(KEY_OPENAI_COMPAT_USE_OPENROUTER) ?: true
        val openAiCompatModels = parseOpenAiCompatModels(prefs.getString(KEY_OPENAI_COMPAT_MODELS))
        return AgentSettings(
            ...
            openAiCompatibleUrl = prefs.getString(KEY_OPENAI_COMPAT_URL).orEmpty().trim().ifBlank { defaultOpenAiCompatUrl() },
            openAiCompatibleUseOpenRouter = useOpenRouter,
            openAiCompatibleModels = openAiCompatModels,
            openAiCompatibleModel = prefs.getString(KEY_OPENAI_COMPAT_MODEL).orEmpty().trim().ifBlank { defaultOpenAiCompatModel() },
            ...
            openAiCompatibleTimeoutSeconds = (prefs.getInteger(KEY_OPENAI_COMPAT_TIMEOUT) ?: defaultOpenAiCompatTimeoutSeconds())
                .coerceIn(30, 3600),
            promptTemplateProfile = prefs.getString(KEY_PROMPT_TEMPLATE_PROFILE).orEmpty().ifBlank { defaultPromptTemplateProfile() },
            promptTemplateCustom = prefs.getString(KEY_PROMPT_TEMPLATE_CUSTOM).orEmpty().ifBlank { defaultPromptTemplateCustom() },
            ...
            preferredBackendId = (prefs.getString(KEY_PREFERRED_BACKEND) ?: "openai-compatible").trim(),
            privacyMode = privacy,
            ...
            passiveAiEnabled = prefs.getBoolean(KEY_PASSIVE_AI_ENABLED) ?: true,
    ...
    fun defaultSettings(): AgentSettings {
        return AgentSettings(
            ...
            lmStudioApiKey = "",
            lmStudioHeaders = "",
            openAiCompatibleUrl = defaultOpenAiCompatUrl(),
            openAiCompatibleUseOpenRouter = true,
            openAiCompatibleModels = defaultOpenAiCompatModels(),
            openAiCompatibleModel = defaultOpenAiCompatModel(),
            openAiCompatibleApiKey = "",
            openAiCompatibleHeaders = "",
            openAiCompatibleTimeoutSeconds = defaultOpenAiCompatTimeoutSeconds(),
            promptTemplateProfile = defaultPromptTemplateProfile(),
            promptTemplateCustom = defaultPromptTemplateCustom(),
            ...
            preferredBackendId = "openai-compatible",
    ...
    fun save(settings: AgentSettings) {
        ...
        prefs.setBoolean(KEY_OPENAI_COMPAT_USE_OPENROUTER, settings.openAiCompatibleUseOpenRouter)
        prefs.setString(
            KEY_OPENAI_COMPAT_MODELS,
            serializeOpenAiCompatModels(normalizeOpenAiCompatModels(settings.openAiCompatibleModels, settings.openAiCompatibleModel))
        )
        ...
        prefs.setString(KEY_PROMPT_TEMPLATE_PROFILE, settings.promptTemplateProfile)
        prefs.setString(KEY_PROMPT_TEMPLATE_CUSTOM, settings.promptTemplateCustom)
    ...
    companion object {
        ...
        private const val KEY_OPENAI_COMPAT_USE_OPENROUTER = "openai.compat.useOpenRouter"
        private const val KEY_OPENAI_COMPAT_MODELS = "openai.compat.models"
        ...
        private const val KEY_PROMPT_TEMPLATE_PROFILE = "prompt.library.profile"
        private const val KEY_PROMPT_TEMPLATE_CUSTOM = "prompt.library.custom"
        ...
        internal fun defaultOpenAiCompatUrl(): String {
            return "https://openrouter.ai/api/v1"
        }

        internal fun defaultOpenAiCompatModel(): String {
            return "deepseek/deepseek-chat-v3.2"
        }

        internal fun defaultOpenAiCompatModels(): List<String> {
            return listOf(
        ...
        private fun parseOpenAiCompatModels(raw: String?): List<String> {
            ...
            if (parsed.isEmpty()) {
                merged.addAll(defaultOpenAiCompatModels())
            } else {
                merged.addAll(parsed)
            }
            val defaultModel = defaultOpenAiCompatModel().trim()
            if (defaultModel.isNotBlank()) merged.add(defaultModel)
            return merged.toList()
        }

        private fun normalizeOpenAiCompatModels(models: List<String>, selected: String): List<String> {
            val merged = LinkedHashSet<String>()
            models.map { it.trim() }.filter { it.isNotBlank() }.forEach { merged.add(it) }
            val chosen = selected.trim()
            if (chosen.isNotBlank()) merged.add(chosen)
            if (merged.isEmpty()) merged.addAll(defaultOpenAiCompatModels())
            return merged.toList()
        }

        private fun serializeOpenAiCompatModels(models: List<String>): String {
        ...
        private fun defaultPromptTemplateProfile(): String {
            return PromptTemplateLibrary.VULN_ANALYSIS_ONLY
        }

        private fun defaultPromptTemplateCustom(): String {
            return ""
        }
