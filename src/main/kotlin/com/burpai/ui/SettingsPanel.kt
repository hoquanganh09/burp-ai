...
import com.burpai.ui.panels.BackendConfigPanel
import com.burpai.ui.panels.BackendConfigState
...
class SettingsPanel(
    ...
    private val backendConfigPanel = BackendConfigPanel(
        BackendConfigState(
            ...
            openAiCompatUseOpenRouter = settings.openAiCompatibleUseOpenRouter,
            openAiCompatModels = settings.openAiCompatibleModels,
    ...
    private val promptTemplateProfile = JComboBox(PromptTemplateLibrary.options.toTypedArray()).apply {
        selectedItem = settings.promptTemplateProfile
        preferredSize = java.awt.Dimension(240, preferredSize.height)
        maximumSize = java.awt.Dimension(240, preferredSize.height)
    }
    private val promptTemplateCustom = JTextArea(settings.promptTemplateCustom, 3, 20)
    private val promptTemplateHint = JLabel("Selected template is prepended before every AI request.")
    ...
    init {
        ...
        applyAreaStyle(promptTemplateCustom)

        styleCombo(privacyMode)
        styleCombo(profilePicker)
        styleCombo(promptTemplateProfile)
        ...
        promptTemplateProfile.toolTipText = "Choose an output template for all AI responses."
        promptTemplateCustom.toolTipText = "Used only when template profile is set to Custom."
        promptTemplateHint.font = UiTheme.Typography.body
        promptTemplateHint.foreground = UiTheme.Colors.onSurfaceVariant
        promptTemplateProfile.addActionListener { updatePromptTemplateControls() }
        updatePromptTemplateControls()
    ...
    fun preferredBackendId(): String {
        return preferredBackend.selectedItem as? String ?: "openai-compatible"
    }
    ...
    fun currentSettings(): AgentSettings {
        ...
        return AgentSettings(
            ...
            openAiCompatibleUseOpenRouter = backendState.openAiCompatUseOpenRouter,
            openAiCompatibleModels = backendState.openAiCompatModels,
            ...
            promptTemplateProfile = promptTemplateProfile.selectedItem as? String
                ?: PromptTemplateLibrary.VULN_ANALYSIS_ONLY,
            promptTemplateCustom = promptTemplateCustom.text.trim(),
    ...
    private fun applySettingsToUi(updated: AgentSettings) {
        ...
        backendConfigPanel.applyState(
            BackendConfigState(
                ...
                openAiCompatUseOpenRouter = updated.openAiCompatibleUseOpenRouter,
                openAiCompatModels = updated.openAiCompatibleModels,
        ...
        promptTemplateProfile.selectedItem = updated.promptTemplateProfile
        promptTemplateCustom.text = updated.promptTemplateCustom
        updatePromptTemplateControls()
    ...
    private fun applyAndSaveSettings(updated: AgentSettings) {
        ...
        activeAiScanner.maxRiskLevel = com.burpai.scanner.PayloadRisk.fromString(updated.activeAiMaxRiskLevel)
        activeAiScanner.scopeOnly = updated.activeAiScopeOnly
        activeAiScanner.scanMode = com.burpai.scanner.ScanMode.fromString(updated.activeAiScanMode)
    ...
    private fun passiveAiScannerSection(): JPanel {
        ...
        actionsPanel.layout = FlowLayout(FlowLayout.LEFT, 8, 0)
    ...
    private fun showPassiveAiFindingsDialog() {
        ...
        JOptionPane.showMessageDialog(
            dialogParentComponent(),
            configureVerticalScroll(JScrollPane(textArea)),
    ...
    private fun applyActiveAiSettings() {
        ...
        activeAiScanner.maxRiskLevel = com.burpai.scanner.PayloadRisk.fromString(
            activeAiRiskLevelCombo.selectedItem as? String ?: "SAFE"
        )
        activeAiScanner.scopeOnly = activeAiScopeOnly.isSelected
        activeAiScanner.scanMode = com.burpai.scanner.ScanMode.fromString(
    ...
    private fun promptSection(): JPanel {
        ...
        val templateTitle = JLabel("Response template library")
        templateTitle.font = UiTheme.Typography.label
        templateTitle.foreground = UiTheme.Colors.onSurfaceVariant
        templateTitle.border = EmptyBorder(0, 0, 6, 0)
        content.add(templateTitle)

        val templateGrid = formGrid()
        addRowFull(templateGrid, "Template profile", promptTemplateProfile)
        addRowFull(templateGrid, "Custom template", verticalScroll(promptTemplateCustom))
        addRowFull(templateGrid, "Note", promptTemplateHint)
        content.add(templateGrid)

        val divider = JPanel()
        divider.background = UiTheme.Colors.outlineVariant
        divider.maximumSize = java.awt.Dimension(Int.MAX_VALUE, 1)
        divider.preferredSize = java.awt.Dimension(1, 1)
        divider.minimumSize = java.awt.Dimension(1, 1)
        divider.border = EmptyBorder(10, 0, 10, 0)
        content.add(divider)
        ...
        addRowFull(requestGrid, "Find vulnerabilities", verticalScroll(promptRequest))
        addRowFull(requestGrid, "Analyze this request", verticalScroll(promptSummary))
        addRowFull(requestGrid, "Explain JS", verticalScroll(promptJs))
        addRowFull(requestGrid, "Access control", verticalScroll(promptAccessControl))
        addRowFull(requestGrid, "Login sequence", verticalScroll(promptLoginSequence))
        ...
        addRowFull(issueGrid, "Analyze this issue", verticalScroll(promptIssueAnalyze))
        addRowFull(issueGrid, "Generate PoC & validate", verticalScroll(promptIssuePoc))
        addRowFull(issueGrid, "Impact & severity", verticalScroll(promptIssueImpact))
        addRowFull(issueGrid, "Full report", verticalScroll(promptIssueFull))
    ...
    private fun tokenPanel(): JPanel {
        val panel = JPanel()
        panel.layout = FlowLayout(FlowLayout.LEFT, 8, 0)
    ...
    private fun buildMcpToolsPanel(): JPanel {
        val toolsPanel = object : JPanel(), Scrollable {
            override fun getPreferredScrollableViewportSize(): java.awt.Dimension = preferredSize
        ...
        McpToolCatalog.all().groupBy { it.category }.forEach { (category, tools) ->
            ...
            val grid = JPanel(java.awt.GridLayout(0, 2, 16, 4))
            grid.background = UiTheme.Colors.surface
        ...
        val scroll = configureVerticalScroll(JScrollPane(toolsPanel))
        ...
        return JPanel(BorderLayout()).apply {
            background = UiTheme.Colors.surface
            val actions = JPanel()
            actions.layout = FlowLayout(FlowLayout.LEFT, 8, 0)
    ...
    private fun updatePromptTemplateControls() {
        val selected = promptTemplateProfile.selectedItem as? String ?: PromptTemplateLibrary.VULN_ANALYSIS_ONLY
        val isCustom = selected == PromptTemplateLibrary.CUSTOM
        promptTemplateCustom.isEditable = isCustom
        promptTemplateCustom.isEnabled = isCustom
        val disabledBg = UiTheme.Colors.inputBackground.darker()
        promptTemplateCustom.background = if (isCustom) UiTheme.Colors.inputBackground else disabledBg
        promptTemplateCustom.foreground = if (isCustom) UiTheme.Colors.inputForeground else UiTheme.Colors.onSurfaceVariant
    }
